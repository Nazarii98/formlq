...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    return subscribeToUsers((all) => {
      setUsers(all.sort((a, b) => (b.createdAt?.seconds ?? 0) - (a.createdAt?.seconds ?? 0)));
      setIsLoading(false);
    });
  }, []);
  ...
  async function toggleRole(uid: string, current: "student" | "editor") {
    const next = current === "editor" ? "student" : "editor";
    setUpdatingRole(uid);
    try {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      <div className="relative">
        ...
        <input
          value={search}
          onChange={(e) => { setSearch(e.target.value); setStudentsVisible(10); }}
        ...
        <div className="space-y-4">
          ...
          {offlineStudents.length > 0 && (
            <div className="space-y-2">
              ...
              {offlineStudents.slice(0, studentsVisible).map((u) => <UserRow key={u.uid} u={u} />)}
              {offlineStudents.length > studentsVisible && (
                ...
                >
                  Показати ще {Math.min(10, offlineStudents.length - studentsVisible)} з {offlineStudents.length - studentsVisible}
                </button>
              )}
            </div>
          )}
