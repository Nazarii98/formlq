...
export default function AdminTestsPage() {
  ...
  function performReorder(src: number, dst: number) {
    const next = [...tests];
    const [moved] = next.splice(src, 1);
    next.splice(dst, 0, moved);
    const updated = next.map((t, i) => ({ ...t, order: i }));
    queryClient.setQueryData(["all-tests"], updated);
    Promise.all(updated.map((t, i) => updateTest(t.id, { order: i })));
  }
    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Створіть перший тест"
        />
      ) : (
              ...
              >
                ...
                <div className="flex [@media(hover:hover)]:hidden flex-col shrink-0">
                  ...
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      if (i < tests.length - 1) performReorder(i, i + 1);
                    }}
                ...
                >
                  ...
                  >
                    ...
                    <button
                      onClick={(e) =>
                        handleTogglePublished(e, test.id, test.published)
                      }
