...
export default function TutorLayout({
  ...
  // Editors may also enter the tutor area (admins manage everything).
  const allowed = userProfile?.role === "tutor" || userProfile?.role === "editor";
  ...
  const firstName = (userProfile?.displayName ?? user?.displayName ?? "Вчитель").split(" ")[0];
  const isRoot = pathname === "/tutor";
        ...
        >
          ...
          {title && !isRoot ? (
            ...
            <div className="flex flex-col justify-center flex-1 min-w-0">
              <span className="text-xs text-muted-foreground leading-tight">Кабінет вчителя</span>
