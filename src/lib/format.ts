...
export function formatDate(ts: Timestamp | null, opts?: Intl.DateTimeFormatOptions): string {
  if (!ts) return "—";
  return ts.toDate().toLocaleString("uk", opts ?? {
    day: "numeric",
    month: "short",
    year: "numeric",
    hour: "2-digit",
...
export function timeAgo(date: Date | { seconds: number } | null | undefined): string {
  if (!date) return "—";
